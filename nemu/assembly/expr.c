...
# 25 "src/monitor/sdb/expr.c" 2





# 29 "src/monitor/sdb/expr.c"
extern const char*regs[];
word_t paddr_read(paddr_t addr, int len);

static inline 
# 32 "src/monitor/sdb/expr.c" 3 4
             _Bool 
# 32 "src/monitor/sdb/expr.c"
...
static inline 
# 57 "src/monitor/sdb/expr.c" 3 4
             _Bool 
# 57 "src/monitor/sdb/expr.c"
                  is_tk_expr_type(int type) {
    switch (type) {
        case TK_EQ:
        case TK_NE:
        case TK_LE:
        ...
        case '/':
            return 
# 74 "src/monitor/sdb/expr.c" 3 4
                  0
# 74 "src/monitor/sdb/expr.c"
                       ;
            break;
        case TK_HEX:
        case TK_REG:
        ...
        case TK_RIGHT_BRACKET:
            return 
# 80 "src/monitor/sdb/expr.c" 3 4
                  1
# 80 "src/monitor/sdb/expr.c"
                      ;
            break;
        default:
            do { if (!(
# 83 "src/monitor/sdb/expr.c" 3 4
           0
# 83 "src/monitor/sdb/expr.c"
           )) { (fflush(
# 83 "src/monitor/sdb/expr.c" 3 4
           stdout
# 83 "src/monitor/sdb/expr.c"
...
static inline 
# 88 "src/monitor/sdb/expr.c" 3 4
             _Bool 
# 88 "src/monitor/sdb/expr.c"
                  is_unary_oper(int type) {
    switch (type) {
        case TK_NEG:
        case TK_DEREF:
            return 
# 92 "src/monitor/sdb/expr.c" 3 4
                  1
# 92 "src/monitor/sdb/expr.c"
                      ;
            break;
        default:
            return 
...
# 145 "src/monitor/sdb/expr.c" 3 4
                                         1
# 145 "src/monitor/sdb/expr.c"
                                                     );
    if (ret != 0) {
      regerror(ret, &re[i], error_msg, 128);
      do { if (!(0)) { (fflush(
# 148 "src/monitor/sdb/expr.c" 3 4
     stdout
# 148 "src/monitor/sdb/expr.c"
     ), fprintf(
# 148 "src/monitor/sdb/expr.c" 3 4
     stderr
# 148 "src/monitor/sdb/expr.c"
...
# 148 "src/monitor/sdb/expr.c"
     "0"
# 148 "src/monitor/sdb/expr.c" 3 4
     , "src/monitor/sdb/expr.c", 148, __extension__ __PRETTY_FUNCTION__); }))
# 148 "src/monitor/sdb/expr.c"
     ; } } while (0);
    }
  }
...
# 162 "src/monitor/sdb/expr.c"
    ...
    for (i = 0; i < (int)(sizeof(rules) / sizeof(rules[0])); i ++) {
      if (regexec(&re[i], e + position, 1, &pmatch, 0) == 0 && pmatch.rm_so == 0) {
        ...
        if ( nr_token >= 4096 )
        {
            do { printf("\33[1;34m" "[%s:%d %s] " "nr_token 超出了token数组大小限制，实际长度: %d\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 190, __func__, nr_token); do { extern FILE* log_fp; extern 
# 190 "src/monitor/sdb/expr.c" 3 4
           _Bool 
# 190 "src/monitor/sdb/expr.c"
           log_enable(); if (log_enable() && log_fp != 
# 190 "src/monitor/sdb/expr.c" 3 4
           ((void *)0)
# 190 "src/monitor/sdb/expr.c"
           ) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "nr_token 超出了token数组大小限制，实际长度: %d\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 190, __func__, nr_token); fflush(log_fp); } } while (0); } while (0);
            return 
# 191 "src/monitor/sdb/expr.c" 3 4
                  0
# 191 "src/monitor/sdb/expr.c"
                       ;
        ...
        if ( substr_len >= 128 )
        {
            do { printf("\33[1;34m" "[%s:%d %s] " "substr_len 超出了Token结构体限制，实际长度: %d\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 197, __func__, substr_len); do { extern FILE* log_fp; extern 
# 197 "src/monitor/sdb/expr.c" 3 4
           _Bool 
# 197 "src/monitor/sdb/expr.c"
           log_enable(); if (log_enable() && log_fp != 
# 197 "src/monitor/sdb/expr.c" 3 4
           ((void *)0)
# 197 "src/monitor/sdb/expr.c"
           ) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "substr_len 超出了Token结构体限制，实际长度: %d\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 197, __func__, substr_len); fflush(log_fp); } } while (0); } while (0);
            return 
# 198 "src/monitor/sdb/expr.c" 3 4
                  0
# 198 "src/monitor/sdb/expr.c"
                       ;
        ...
        switch (rules[i].token_type) {
            ...
            case '*':
                if ( nr_token == 0 || (!is_tk_expr_type(tokens[nr_token - 1].type)) )
                {
                    t.type = TK_DEREF;
                }
                else
            ...
            default:
                ...
                for ( int i = 0; i < substr_len; i += 1 )
                    t.str[i] = substr_start[i];
    ...
    if (i == (int)(sizeof(rules) / sizeof(rules[0]))) {
      ;
      return 
# 258 "src/monitor/sdb/expr.c" 3 4
            0
# 258 "src/monitor/sdb/expr.c"
                 ;
    }
  }
  ;
...
word_t read_expr(Token *, char **msg_addr);
word_t perform_unary_oper(Token oper_token, word_t operand, char **msg_addr);

int read_operator(Token *dtokens, char **msg_addr) {
    if ( token_index >= nr_dtoken )
        { if ( is_err( "read_operator: 超出TOKEN_NUMtoken数量限制" ) ) { *msg_addr = "read_operator: 超出TOKEN_NUMtoken数量限制"; return -1; } };



    switch ( dtokens[token_index].type ) {
        case TK_EQ:
        case TK_NE:
        case TK_LE:
        case TK_GE:
        ...
        default:
            { if ( is_err( "read_operand: 没有读到操作符" ) ) { *msg_addr = "read_operand: 没有读到操作符"; return -1; } };
            return -1;
            break;
    }
}
...
word_t read_operand(Token *dtokens, char **msg_addr) {

    Token token = dtokens[token_index];


    token_index += 1;
    ;

    if ( is_unary_oper(token.type) )
    {
        char *operand_msg = 
# 311 "src/monitor/sdb/expr.c" 3 4
       ((void *)0)
# 311 "src/monitor/sdb/expr.c"
       ;;
        word_t data = read_operand(dtokens, &operand_msg); { if ( is_err( operand_msg ) ) { *msg_addr = operand_msg; return -1; } };

        char *perform_msg = 
# 314 "src/monitor/sdb/expr.c" 3 4
       ((void *)0)
# 314 "src/monitor/sdb/expr.c"
       ;;
        word_t result = perform_unary_oper(token, data, &perform_msg); { if ( is_err( perform_msg ) ) { *msg_addr = perform_msg; return -1; } };
        return result;
    }
    ...
    switch (token.type) {
        case TK_DIGIT:
            ...
            if ( sscanf(token.str, " %u ", &value) <= 0 )
            {
                { if ( is_err( "strtol解析错误" ) ) { *msg_addr = "strtol解析错误"; return -1; } };
            }
            return value;
        ...
        case TK_HEX:
            word_t hex;
            if ( sscanf(token.str, " %x", &hex) <= 0 )
            {
                { if ( is_err( "read_operand: parse hex error" ) ) { *msg_addr = "read_operand: parse hex error"; return -1; } };
            }
            return hex;
            break;
        case TK_REG:
            
# 339 "src/monitor/sdb/expr.c" 3 4
           _Bool 
# 339 "src/monitor/sdb/expr.c"
                success;
            word_t data = isa_reg_str2val(token.str, &success);
            if ( success )
                return data;
            { if ( is_err( "read_operand: parse reg error" ) ) { *msg_addr = "read_operand: parse reg error"; return -1; } };
            return -1;
            break;
        case TK_LEFT_BRACKET:
            char *msg = 
# 347 "src/monitor/sdb/expr.c" 3 4
           ((void *)0)
# 347 "src/monitor/sdb/expr.c"
           ;;
            word_t operand = read_expr(dtokens, &msg); { if ( is_err( msg ) ) { *msg_addr = msg; return -1; } };
            return operand;
            break;
        default:
            { if ( is_err( "read_operand: Found Mismatch operand type" ) ) { *msg_addr = "read_operand: Found Mismatch operand type"; return -1; } };
...
word_t perform_unary_oper(Token oper_token, word_t operand, char **msg_addr) {
    switch (oper_token.type) {
        case TK_DEREF:
            if (__builtin_expect(in_pmem(operand), 1))
                return paddr_read(operand, 4);
            { if ( is_err( "地址不在内存中" ) ) { *msg_addr = "地址不在内存中"; return -1; } };
            return -1;
            break;
        ...
        case TK_NEG:
            ...
            break;
        default:
            do { printf("\33[1;34m" "[%s:%d %s] " "Unknown type: %d, '-' = %d\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 372, __func__, oper_token.type, '-'); do { extern FILE* log_fp; extern 
# 372 "src/monitor/sdb/expr.c" 3 4
           _Bool 
# 372 "src/monitor/sdb/expr.c"
           log_enable(); if (log_enable() && log_fp != 
# 372 "src/monitor/sdb/expr.c" 3 4
           ((void *)0)
# 372 "src/monitor/sdb/expr.c"
           ) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Unknown type: %d, '-' = %d\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 372, __func__, oper_token.type, '-'); fflush(log_fp); } } while (0); } while (0);
            { if ( is_err( "perform_bin_oper: Unknown Operator" ) ) { *msg_addr = "perform_bin_oper: Unknown Operator"; return -1; } };
            return -1;
    }
    { if ( is_err( "perform_unary_oper：不应该到达的控制流" ) ) { *msg_addr = "perform_unary_oper：不应该到达的控制流"; return -1; } };
    return -1;
}

word_t perform_bin_oper(Token oper_token, word_t operand1, word_t operand2, char **msg_addr) {
    switch (oper_token.type) {
        case '+':
        ...
        case '/':
            ...
            if ( operand2 == 0 ) {
                ...
                { if ( is_err( "perform_bin_oper: Zero Division Exception!" ) ) { *msg_addr = "perform_bin_oper: Zero Division Exception!"; return -1; } };
            }
        ...
        case TK_AND:
            return (
# 425 "src/monitor/sdb/expr.c" 3 4
                   _Bool
# 425 "src/monitor/sdb/expr.c"
                       )(operand1 && operand2);
            break;
        case TK_OR:
            return (
# 428 "src/monitor/sdb/expr.c" 3 4
                   _Bool
# 428 "src/monitor/sdb/expr.c"
                       )(operand1 || operand2);
            break;
        case TK_BITOR:
            do { printf("\33[1;34m" "[%s:%d %s] " "operand1 = 0x%x, operand2 = 0x%x\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 431, __func__, operand1, operand2); do { extern FILE* log_fp; extern 
...
# 431 "src/monitor/sdb/expr.c" 3 4
           ((void *)0)
# 431 "src/monitor/sdb/expr.c"
           ) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "operand1 = 0x%x, operand2 = 0x%x\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 431, __func__, operand1, operand2); fflush(log_fp); } } while (0); } while (0);
            return (operand1 | operand2);
            break;
        case TK_BITAND:
            return operand1 & operand2;
            break;
        default:
            do { printf("\33[1;34m" "[%s:%d %s] " "Unknown type: %d, '-' = %d\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 438, __func__, oper_token.type, '-'); do { extern FILE* log_fp; extern 
# 438 "src/monitor/sdb/expr.c" 3 4
           _Bool 
# 438 "src/monitor/sdb/expr.c"
           log_enable(); if (log_enable() && log_fp != 
# 438 "src/monitor/sdb/expr.c" 3 4
           ((void *)0)
# 438 "src/monitor/sdb/expr.c"
           ) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Unknown type: %d, '-' = %d\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 438, __func__, oper_token.type, '-'); fflush(log_fp); } } while (0); } while (0);
            { if ( is_err( "perform_bin_oper: Unknown Operator" ) ) { *msg_addr = "perform_bin_oper: Unknown Operator"; return -1; } };
...
word_t read_expr(Token dtokens[], char **msg_addr)
{
    word_t operand1;
    if ( dtokens[token_index].type == TK_LEFT_BRACKET )
    {
        ...
        char *msg = 
# 456 "src/monitor/sdb/expr.c" 3 4
       ((void *)0)
# 456 "src/monitor/sdb/expr.c"
       ;;
        operand1 = read_expr(dtokens, &msg);
        { if ( is_err( msg ) ) { *msg_addr = msg; return -1; } };
    ...
    {
        char *msg = 
# 467 "src/monitor/sdb/expr.c" 3 4
       ((void *)0)
# 467 "src/monitor/sdb/expr.c"
       ;;
        operand1 = read_operand(dtokens, &msg);
        { if ( is_err( msg ) ) { *msg_addr = msg; return -1; } };
    }

    char *read_op_msg = 
# 472 "src/monitor/sdb/expr.c" 3 4
   ((void *)0)
# 472 "src/monitor/sdb/expr.c"
   ;;
    int op_index = read_operator(dtokens, &read_op_msg);
    if ( is_err(read_op_msg) )
    {
        return operand1;
    }
    else
    {
        ...
        char *msg_operand2 = 
# 481 "src/monitor/sdb/expr.c" 3 4
       ((void *)0)
# 481 "src/monitor/sdb/expr.c"
       ;;
        word_t operand2 = read_expr(dtokens, &msg_operand2); { if ( is_err( msg_operand2 ) ) { *msg_addr = msg_operand2; return -1; } };


        char *msg_perform = 
# 485 "src/monitor/sdb/expr.c" 3 4
       ((void *)0)
# 485 "src/monitor/sdb/expr.c"
       ;;
        word_t value = perform_bin_oper(dtokens[op_index], operand1, operand2, &msg_perform); { if ( is_err( msg_perform ) ) { *msg_addr = msg_perform; return -1; } };
        return value;
    }

    { if ( is_err( "read_expr: 控制流异常，进入非法分支" ) ) { *msg_addr = "read_expr: 控制流异常，进入非法分支"; return -1; } };
    return -1;
}


# 494 "src/monitor/sdb/expr.c" 3 4
_Bool 
# 494 "src/monitor/sdb/expr.c"
    is_pair(int start, int end) {
    int depth = 0;
        ...
        switch (tokens[i].type) {
            case TK_RIGHT_BRACKET:
                if ( depth == 0 ) {
                    if ( i == end )
                        return 
# 502 "src/monitor/sdb/expr.c" 3 4
                              1
# 502 "src/monitor/sdb/expr.c"
                                  ;
                    else
                        return 
# 504 "src/monitor/sdb/expr.c" 3 4
                              0
# 504 "src/monitor/sdb/expr.c"
                                   ;
                }
                else
                    depth -= 1;
    ...
    return 
# 514 "src/monitor/sdb/expr.c" 3 4
          0
# 514 "src/monitor/sdb/expr.c"
               ;
}

void wrap_bracket(int dpos[], int *pos_len, int start, int end) {
# 527 "src/monitor/sdb/expr.c"
    ;

    if ( end - start <= 2 ) return;

    if (is_pair(start, end))
    {
        return wrap_bracket(dpos, pos_len, start + 1, end - 1);
    }

    int i = end, prior_oper_pos = -1, prior_oper_level = -1, bracket_level = 0;
    for ( ; i >= start; i -= 1 )
    {
        switch ( tokens[i].type ) {
            case TK_RIGHT_BRACKET:
            ...
            case TK_NE:
            case TK_EQ:
                if ( bracket_level == 0 && 5 > prior_oper_level ) {
                    prior_oper_pos = i;
                    prior_oper_level = 5;
                }
                break;
            case TK_BITOR:
                if ( bracket_level == 0 && 4 > prior_oper_level ) {
                    prior_oper_pos = i;
                    prior_oper_level = 4;
                }
                break;
            case TK_BITAND:
                if ( bracket_level == 0 && 3 > prior_oper_level ) {
                    prior_oper_pos = i;
                    prior_oper_level = 3;
                }
                break;
            ...
            case TK_AND:
            case TK_OR:
                if ( bracket_level == 0 && 2 > prior_oper_level ) {
                    prior_oper_pos = i;
                    prior_oper_level = 2;
                }
                break;
            case '*':
            case '/':
                if ( bracket_level == 0 && 1 > prior_oper_level ) {
                    prior_oper_pos = i;
                    prior_oper_level = 1;
                }
                break;
    ...
    switch ( prior_oper_pos ) {
        case -1:
            return;
            break;
        default:
            if ( prior_oper_pos - start > 1 )
            {
                dpos[*pos_len] = start;
                *pos_len += 1;
                dpos[*pos_len] = prior_oper_pos;
                *pos_len += 1;
            }
            if ( end - prior_oper_pos > 1 )
            {
                dpos[*pos_len] = prior_oper_pos + 1;
                *pos_len += 1;
                dpos[*pos_len] = end + 1;
                *pos_len += 1;
            }
    ...
    wrap_bracket(dpos, pos_len, start, prior_oper_pos - 1);
    wrap_bracket(dpos, pos_len, prior_oper_pos + 1, end);
}

static inline void fillin(Token *decorated_tokens, int *decorated_pos, int pos_len) {

    for ( int i = 0; i <= nr_token; i += 1 )
    {
        for ( int j = 0; j < pos_len; j += 1 )
        {
            if ( decorated_pos[j] == i ) {
                if ( j % 2 == 0 )
                {

                    Token left = {.type = TK_LEFT_BRACKET, .str = "("};
                    decorated_tokens[nr_dtoken] = left;
                    nr_dtoken += 1;
                }
                else
                {

                    Token right = {.type = TK_RIGHT_BRACKET, .str = ")"};
                    decorated_tokens[nr_dtoken] = right;
                    nr_dtoken += 1;
                }
            }
        }
        if ( i != nr_token ) {
            decorated_tokens[nr_dtoken] = tokens[i];
            nr_dtoken += 1;
        }
    }
}
...
word_t expr(char *e, 
# 640 "src/monitor/sdb/expr.c" 3 4
                    _Bool 
# 640 "src/monitor/sdb/expr.c"
                         *success) {
    if (!make_token(e)) {
        *success = 
# 642 "src/monitor/sdb/expr.c" 3 4
                  0
# 642 "src/monitor/sdb/expr.c"
                       ;
        return -1;
    }
    ...
    Token *decorated_tokens = (Token *)malloc(sizeof(Token) * (4096 + nr_token));
    int *decorated_pos = (int *)malloc(sizeof(int) * (4096 + nr_token));

    int pos_len = 0;
    wrap_bracket(decorated_pos, &pos_len, 0, nr_token - 1);
    fillin(decorated_tokens, decorated_pos, pos_len);
    ...
    char *msg = 
# 666 "src/monitor/sdb/expr.c" 3 4
   ((void *)0)
# 666 "src/monitor/sdb/expr.c"
   ;;
    word_t value = read_expr(decorated_tokens, &msg);
    if ( is_err(msg) ) {
        *success = 
# 669 "src/monitor/sdb/expr.c" 3 4
                  0
# 669 "src/monitor/sdb/expr.c"
                       ;
        do { printf("\33[1;34m" "[%s:%d %s] " "expr: %s\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 670, __func__, msg); do { extern FILE* log_fp; extern 
# 670 "src/monitor/sdb/expr.c" 3 4
       _Bool 
# 670 "src/monitor/sdb/expr.c"
       log_enable(); if (log_enable() && log_fp != 
# 670 "src/monitor/sdb/expr.c" 3 4
       ((void *)0)
# 670 "src/monitor/sdb/expr.c"
       ) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "expr: %s\n" "\33[0m" "\n", "src/monitor/sdb/expr.c", 670, __func__, msg); fflush(log_fp); } } while (0); } while (0);
    } else {
        *success = 
# 672 "src/monitor/sdb/expr.c" 3 4
                  1
# 672 "src/monitor/sdb/expr.c"
                      ;
    }


    free(decorated_pos);
    free(decorated_tokens);
    return value;
}
